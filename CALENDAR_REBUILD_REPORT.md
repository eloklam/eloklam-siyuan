...
- Static feature precheck covers create/update/delete, keyboard navigation, keyboard shortcut metadata, live region metadata, keyboard view switching, double-click creation, quick-copy, occurrence replacement, this-and-future split, recurrence, event tooltips, event summary, search, search clearing, direct date jumping, view switching, drag/drop, timed resize, and all-day duration resize terms.
...
- Calendar render-flow guards for empty date-field setup, date-field creation, month/week/day/schedule modes, keyboard navigation, keyboard shortcut metadata, live region metadata, keyboard view switching, event tooltips, event summary, double-click creation, quick-copy, schedule drag/drop targets, search rerendering, search result count, search clearing, direct date jumping, week-start range calculation, editable event lookup, and drag/drop date offsets are covered by `scripts/calendar-audit.mjs`.
